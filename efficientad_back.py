...
def main():
    ...
    config.cfg = check_yaml(config.cfg)  # check YAML
    teacher_yolo = Model(config.cfg).to(device)
    img = torch.rand(8 if torch.cuda.is_available() else 1, 3, 640, 640).to(device)
    y = teacher_yolo(img, profile=True)
    feature = y[-1]
    print(len(feature))
    ...
    for iteration, (image_st, image_ae), image_penalty in zip(
        ...
        if on_gpu:
            image_st = image_st.cuda()   # image_st shape ： torch.Size([1, 3, 256, 256])
        ...
        with torch.no_grad():
            ...
            print("teacher_output_st shape: ", teacher_output_st.shape)  # teacher_output_st shape:  torch.Size([1, 384, 56, 56])
        ...
        if image_penalty is not None:  # 默认为None
            print("image_penalty shape: ", image_penalty.shape)  # image_penalty shape:  torch.Size([1, 3, 256, 256]
